...
func gn_TorrDwnd_fileurl(str string) string {
	if str == "" {
		return ""
	}
	return fmt.Sprintf("https://itorrents.org/torrent/%s.torrent", str)
}

func getYts_mg(info_hash string) string {
...
func getYts_fileurl(str string) string {
	if str == "" {
		return ""
	}
	return fmt.Sprintf("https://itorrents.org/torrent/%s.torrent", str)
...
func getKickass_fileurl(str string) string {
	if str == "" {
		return ""
	}
...
func getRarbg_fileurl(str string) string {
	if str == "" {
		return ""
	}
	re := regexp.MustCompile(`(?i)[a-z0-9]{40}`)
